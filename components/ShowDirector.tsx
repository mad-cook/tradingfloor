...
import {freshOffer,type PrivateOffer} from '@/packages/core/private-markets';
...
export function startShow(kind:string,source='Director’s pick',offer?:PrivateOffer){
 const s=useFloor.getState();if(s.show||s.cutaway||!s.snapshot||s.snapshot.paused)return false;
 if(kind.startsWith('private-')&&(!offer||!freshOffer(offer)))return false;
 ...
 const episode:ShowEpisode={id:Date.now(),kind, ...cast,variant:count%2,source,offer,number:++number};showClock.elapsed=0;s.setShow(episode);return true;
}
export default function ShowDirector({enabled}:{enabled:boolean}){
 ...
 if(s.cutaway){if(s.show)s.setShow(null);idle=0;next=16000;}
  ...
  }else{idle+=delta;if(idle>=next||(pending&&idle>6000)){const offers=s.privateMarkets?.offers.filter(o=>freshOffer(o))??[];const lastVisit=s.showHistory.find(h=>h.kind.startsWith('private-'));if(offers.length&&(!lastVisit||Date.now()-lastVisit.id>240000)){const offer=offers[number%offers.length];startShow(offer.decision==='WATCHLIST'?'private-watch':'private-pass','PreStocks · research only',offer);}else{const kind=chooseShow(s.showHistory.map(h=>h.kind),pending);startShow(kind,pending?'The gallery: '+pending:'Director’s pick');}pending=null;}}
 }
