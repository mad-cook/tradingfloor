...
export class FloorSound{
 private context:AudioContext;private master:GainNode;private enabled=false;private bedGain:GainNode|null=null;
 ...
 async setEnabled(enabled:boolean){await this.context.resume();this.enabled=enabled;this.master.gain.setTargetAtTime(enabled?.8:0,this.context.currentTime,.08);
  ...
  else{this.queue=[];for(const source of this.sources)try{source.stop();}catch{};useFloor.getState().clearVoices();}this.debug();
 }
 ...
 private enqueue(r:Request){
  const existing=this.queue.findIndex(q=>q.desk===r.desk);
 ...
 private async speak(r:Request){
  const line=this.picker.pick(this.manifest[r.desk]??[],r.category);if(!line)return;this.active++;
  try{
   ...
   source.onended=()=>{this.active--;this.sources.delete(source);useFloor.getState().removeVoice(r.desk);
    if(this.enabled&&r.chapter==='challenge'&&r.replyDesk)this.enqueue({desk:r.replyDesk,category:'reply',kind:'BANTER',at:performance.now()+700,priority:5,chapter:'reply'});
    else if(this.enabled&&r.chapter==='reply')this.enqueue({desk:'principal',category:'boss',kind:'BOSS_CALL',at:performance.now()+900,priority:6});
    this.debug();};
