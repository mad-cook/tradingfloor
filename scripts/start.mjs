...
function launch(args){const child=spawn(process.execPath,args,{env,stdio:'inherit',windowsHide:true});children.add(child);child.on('error',()=>stop(1));child.on('exit',(code)=>{children.delete(child);if(!stopping)stop(code||1);else if(!children.size)process.exit(exitCode);});return child;}
...
if(!ready)stop(1);else if(!stopping)launch(['node_modules/next/dist/bin/next','start','-H','0.0.0.0','-p',env.PORT??'3100']);
