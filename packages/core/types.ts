...
export type Ticket={id:number;at:number;deskId:string;symbol:string;side:'BUY'|'SELL';usd:number;price:number;qty:number;paper:true;signature:null};
export type Snapshot={mode:'demo';connected:boolean;session:number;now:number;marketOpen:boolean;paused:boolean;killed:boolean;cash:number;nav:number;openNav:number;turnover:number;benchmarkOpen:number;desks:Desk[];events:FloorEvent[];tickets:Ticket[];curve:{at:number;nav:number}[];archive:Desk[]};
