...
export type ShowEpisode={id:number;kind:string;lead:string;rival:string;variant:number;source:string;number:number};
...
export function chooseShow(recent:string[],reaction:Reaction|null,random=Math.random){
 const pool=reaction==='back'?['victory','speech']:reaction==='doubt'?['audit','blame']:reaction==='chaos'?['printer','outage','phones','coffee']:SHOWS.map(s=>s.id);
...
export function showSpeaker(episode:ShowEpisode,beat:number){const role=definition(episode.kind).beats[beat]?.speaker;return role==='principal'?'principal':role==='rival'?episode.rival:episode.lead;}
